...
// The in-process tests wire the client straight into the real server half, so
// they only build where that half does (see package server). TestSSHArgv and
// anything else that needs no server stays in client_test.go, which builds
// everywhere the client does.
package client
...
func inProcess(t *testing.T) (*Client, *server.Server, string) {
	...
	cl := &Client{Iface: "wg0", Exec: func(op string, reqJSON []byte) ([]byte, error) {
		...
		case protocol.OpRename:
			return json.Marshal(s.Rename(req))
		}
...
// TestClientRename walks the rename round-trip: the label moves while the key
// stays put, and the duplicate/not-found refusals surface as errors carrying the
// protocol code.
func TestClientRename(t *testing.T) {
	cl, _, _ := inProcess(t)
	if _, _, err := cl.Add(AddOptions{Name: "phone"}); err != nil {
		t.Fatal(err)
	}
	if _, _, err := cl.Add(AddOptions{Name: "laptop"}); err != nil {
		t.Fatal(err)
	}
	before, err := cl.List()
	if err != nil {
	...
	rr, err := cl.Rename("phone", "телефон")
	if err != nil {
		t.Fatalf("Rename: %v", err)
	}
	if rr.Renamed == nil || rr.Renamed.From != "phone" || rr.Renamed.To != "телефон" {
		t.Fatalf("renamed = %+v", rr.Renamed)
	}

	// Same peer, same key, same address — only the label changed.
	after, err := cl.List()
	if err != nil {
		t.Fatal(err)
	}
	if len(after.Peers) != len(before.Peers) {
		t.Fatalf("peer count changed: %d → %d", len(before.Peers), len(after.Peers))
	}
	var renamed *protocol.PeerInfo
	for i := range after.Peers {
		if after.Peers[i].Name == "телефон" {
			renamed = &after.Peers[i]
		}
		if after.Peers[i].Name == "phone" {
			t.Errorf("old name survived: %+v", after.Peers[i])
		}
	}
	if renamed == nil {
		t.Fatalf("renamed peer missing: %+v", after.Peers)
	}
	if renamed.PublicKey != before.Peers[0].PublicKey || renamed.IP != before.Peers[0].IP {
		t.Errorf("rename changed identity: %+v, want key/ip of %+v", renamed, before.Peers[0])
	}

	// Renaming onto a name in use → name_taken.
	if _, err := cl.Rename("телефон", "laptop"); err == nil || !strings.Contains(err.Error(), protocol.ErrNameTaken) {
		t.Errorf("rename onto an existing name err = %v, want name_taken", err)
	}
	// Renaming a peer that does not exist → not_found.
	if _, err := cl.Rename("ghost", "x"); err == nil || !strings.Contains(err.Error(), protocol.ErrNotFound) {
		t.Errorf("rename ghost err = %v, want not_found", err)
	}
	// An unwritable target name is caught locally, before any round-trip.
	if _, err := cl.Rename("телефон", "bad\nname"); err == nil || !strings.Contains(err.Error(), "invalid new name") {
		t.Errorf("rename to a multi-line name err = %v, want a local validation error", err)
	}
}
