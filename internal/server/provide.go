...
type ProvideOptions struct {
	...
	// full-tunnel; empty DNS / zero MTU are written as "unset" (the client omits
	// the line and keeps its own resolver / lets wg-quick derive the MTU).
	AllowedIPs []string
	DNS        []string
	MTU        int
...
func Provide(opts ProvideOptions) protocol.ProvideResponse {
	...
	sidecarText := renderSidecar(opts.Iface, confPath, subnet, serverAddr, allowedIPs, opts.DNS, opts.MTU, endpoints)
...
//
// DNS and MTU are omitted when unset (empty / 0): the server is not a resolver,
// and an unset MTU lets the client's wg-quick derive the right one from its route
// (the wireguard-over-ethernet default). Each is left as an explanatory comment
// so the admin sees the knob exists.
func renderSidecar(iface, confPath string, subnet netip.Prefix, serverAddr netip.Addr, allowedIPs, dns []string, mtu int, endpoints []protocol.Endpoint) string {
	var b strings.Builder
	...
	fmt.Fprintf(&b, "allowed_ips          = [%s]%s\n", quoteJoin(allowedIPs), tunnelComment(allowedIPs))
